...
2. **No hay validación de presencia por el gestor ni franja previa (decisión de Leo, 2026-09-23).** Desde que se crea la entrega hasta su `fin`, el enlace (`/r/[token]` o `/mi-entrega/[id]`) abre el acta y deja comenzar; diligenciarla sin haber visto el espacio queda a criterio de quien recibe. A la hora de inicio llega un correo que lo recuerda si aún no confirmó. `minutos_vigencia_qr_antes` ya no se lee. **Riesgo aceptado:** la constancia prueba la cuenta que confirmó, no la presencia física ni el momento de la visita.
...
- **QR/enlace vigente desde que se crea la entrega hasta el `fin`** del evento (hasta el 2026-09-23 era desde `inicio − 30 min`), y **token derivado** con HMAC del id (§3), no aleatorio almacenado.
