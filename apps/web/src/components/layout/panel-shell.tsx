        ...
        <div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden max-w-80 min-w-0 text-right md:block">
              ...
              <p className="text-xs break-all text-muted-foreground">{usuario.correo}</p>
            </div>
            <Avatar nombre={usuario.nombre} className="size-9" />
            <form action={cerrarSesion}>
              <button
                type="submit"
                aria-label="Cerrar sesión"
                title="Cerrar sesión"
                className="grid size-11 place-items-center rounded-lg text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground"
              >
