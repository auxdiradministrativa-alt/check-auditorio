...
const ATENCION = new Set(['DEVOLUCION_VENCIDA'])
...
export function CentroGestion({
  ...
  const resumen = `${activas.length} ${activas.length === 1 ? 'entrega activa' : 'entregas activas'} y ${historico.length} ${historico.length === 1 ? 'registro' : 'registros'} en el histórico coinciden.`
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8">
      ...
      <OperacionEventos espacios={espacios} eventoId={eventoId} nuevo={nuevo} detalle={detalle} />
      ...
      <div className="flex flex-col gap-4 rounded-card border border-border bg-card p-4 sm:p-6">
        <dl className="grid grid-cols-2 gap-4 sm:gap-6 lg:grid-cols-4">
          {[
            ['Entregas activas', asignaciones.filter((a) => !CERRADAS.has(a.estado)).length],
      ...
      <Card id="entregas" aria-labelledby="titulo-entregas" className="scroll-mt-16">
        <CardHeader className="flex-row flex-wrap items-center justify-between gap-4">
          <div>
            <h2 id="titulo-entregas" className="text-section">
      ...
      <Card id="historico" aria-labelledby="titulo-historico" className="scroll-mt-16">
        <CardHeader className="flex-row flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Todas las entregas, incluidas devoluciones, anulaciones y eventos expirados.
            </p>
          </div>
          <Button
            variante="secundario"
